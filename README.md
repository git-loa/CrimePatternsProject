...
1) It is the most interpretable and our goal is to study how various socio-economic factors influence the crime rate, not just predicting the crime rate.
2) Multiple linear regression has comparable performance in terms of regular 5-fold cross-validation compared to random forests and XGBoost.
3) Multiple linear regression has much better performance in terms of cross-county and time series cross-validation compared to random forests and XGBoost.
